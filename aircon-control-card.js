class AirconControlCard extends HTMLElement {
  constructor() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .temp-circle-container.glow .glow-bottom { opacity: 0.8; animation: halfGlowPulse 12s infinite ease-in-out; }
        .glow-bottom {
          position: absolute; bottom: -12px; left: 50%; transform: translateX(-50%);
          width: 140px; height: 70px; background: var(--glow-color);
          border-radius: 0 0 70px 70px / 0 0 70px 70px;
          filter: blur(10px); opacity: 0.4; pointer-events: none;
          transition: opacity 0.5s ease; animation: none; z-index: 0;
        }
        .temp-circle {
        ...
        .temp-circle::after {
          content: ''; position: absolute; top: 50px; left: 80px;
        ...
        .clickable-sensor { cursor:pointer; text-decoration:underline; color:inherit; }
        ...
        .styled-room-slider {
          ...
          border-radius:12px; outline:none; transition:background 0.3s ease;
        ...
        .styled-room-slider::-moz-range-thumb {
          width:24px; height:24px; background:var(--glow-color, #1B86EF);
  ...
  setConfig(config){
    ...
    const fanModes=this._hass?.states[config.entity]?.attributes.fan_modes?.length>0
                    ...
                    : fallback;
    let fanHtml='';
    fanModes.forEach(f=>{fanHtml+=`<div class="fan-btn-container">
